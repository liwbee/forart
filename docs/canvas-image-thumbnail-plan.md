...
- 节点数据里保存的是缩略图 URL；删掉 `thumb/*.webp` 后请求该 URL 时会按同名原图就地重建，缓存清理不需要改画布数据。
...
画布预览固定使用缩略图，不随缩放级别切换原图：
- 高倍缩放（400% 以上）曾切换回原图，实测会让渲染进程常驻解码数据与合成位图增长数百 MB，
  且在缩回后也不会释放，直到刷新窗口。该行为已移除。
- 原图只用于查看大图、下载、裁剪和提交给生成接口的参考图。
