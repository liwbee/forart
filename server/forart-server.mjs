...
function importLegacyState(payload) {
  if (!payload || typeof payload !== "object") throw new Error("Invalid legacy payload");
  clearLibraryTables();
  const timestamp = nowIso();
  ...
  for (const project of projects) {
    const name = validateFileNamePart(project.name || DEFAULT_PROJECT_NAME, "project name");
    if (projectNameExists(name)) throw new Error("Project name must be unique");
    db.prepare(
      "INSERT INTO model_projects (id, name, cover_asset_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?)"
    ).run(
      String(project.id || newId("project")),
      name,
      project.cover_asset_id ? String(project.cover_asset_id) : null,
      String(project.created_at || timestamp),
      String(project.updated_at || timestamp)
    );
  }
  ...
  for (const tag of tags) {
    db.prepare(
      `
      INSERT INTO library_tags (id, kind, project_id, name, color, sort_order, created_at, updated_at)
      VALUES (?, 'model', ?, ?, ?, ?, ?, ?)
      `
    ).run(
      String(tag.id || newId("tag")),
      tag.project_id ? String(tag.project_id) : null,
      String(tag.name || "").trim(),
      String(tag.color || DEFAULT_TAG_COLOR),
      Number(tag.sort_order || 0),
      String(tag.created_at || timestamp),
      String(tag.updated_at || timestamp)
    );
  }

  for (const model of models) {
    const projectId = String(model.project_id || "");
    const name = validateFileNamePart(model.name || DEFAULT_MODEL_NAME, "model name");
    if (modelNameExists(projectId, name)) throw new Error("Model name must be unique within the project");
    db.prepare(
      `
      INSERT INTO model_entries (id, project_id, name, code, gender, cover_image_id, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `
    ).run(
      String(model.id || newId("model")),
      projectId,
      name,
      String(model.code || nextCode(projectId, loadProject(projectId)?.name || DEFAULT_PROJECT_NAME)),
      sanitizeGender(model.gender),
      model.cover_image_id ? String(model.cover_image_id) : null,
      String(model.created_at || timestamp),
      String(model.updated_at || timestamp)
    );
  }

  const legacyModelMap = new Map();
  for (const model of models) {
    legacyModelMap.set(model.id, model);
    const names = normalizeTags(model.tags || []);
    for (const name of names) {
      ...
      if (!tag) {
        ...
        ).run(tagId, name, DEFAULT_TAG_COLOR, 0, timestamp, timestamp);
      ...
      ).run(newId("entrytag"), String(model.id), tag.id, timestamp);
    }
  }

  for (const image of images) {
    const legacyAsset = image.asset_url ? parseDataUrl(image.asset_url) : null;
    let assetId = image.asset_id ? String(image.asset_id) : null;
    if (legacyAsset) {
      const model = models.find((item) => item.id === image.model_id);
      const project = model ? loadProject(model.project_id) : null;
      const modelName = validateFileNamePart(model?.name || DEFAULT_MODEL_NAME, "model name");
      const projectName = validateFileNamePart(project?.name || DEFAULT_PROJECT_NAME, "project name");
      const relDir = path.relative(STORAGE_ROOT, modelDirForNames(projectName, modelName));
      const asset = writeAsset(legacyAsset.buffer, legacyAsset.mimeType, image.filename || "image", {
        source: "legacy-import",
        subdir: relDir,
        filenameStem: `${modelName}_${String(Number(image.sort_order || 0) + 1).padStart(3, "0")}`,
      });
      assetId = asset.id;
    } else if (!assetId) {
      assetId = newId("asset");
    ...
    db.prepare(
      `
      INSERT INTO model_images (id, model_id, asset_id, caption, sort_order, created_at, mime_type, filename)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `
    ).run(
      String(image.id || newId("image")),
      String(image.model_id || ""),
      assetId,
      String(image.caption || ""),
      Number(image.sort_order || 0),
      String(image.created_at || timestamp),
      image.mime_type ? String(image.mime_type) : null,
      image.filename ? String(image.filename) : null
    );
  }
...
function handleModelLibraryApi(req, res, url) {
  ...
  if (method === "POST" && pathname === "/api/model-library/import-legacy") {
    parseJsonBody(req)
      .then((payload) => {
