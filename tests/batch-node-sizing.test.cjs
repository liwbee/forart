...
test('batch nodes default to the height of four complete list rows', () => {
  assert.equal(sizing.BATCH_NODE_DEFAULT_VISIBLE_ROWS, 4);
  assert.equal(sizing.BATCH_NODE_LIST_ROW_HEIGHT, 86);
  assert.equal(sizing.BATCH_NODE_LIST_ROW_GAP, 14);
  assert.equal(sizing.BATCH_NODE_HEIGHT_STEP, 100);
  assert.equal(sizing.BATCH_NODE_DEFAULT_HEIGHT, 478);
  assert.equal(sizing.batchNodeHeightForListRows(5) - sizing.batchNodeHeightForListRows(4), 100);
});
...
test('batch grid previews use the regular 3:4 asset size above an auto-height toolbar', () => {
  ...
  assert.match(canvasCss, /\.rf-action-fission-list\s*\{[^}]*gap:\s*14px;/s);
  assert.match(canvasCss, /\.rf-action-fission-list-card\s*\{[^}]*height:\s*86px;/s);
});

test('batch node resize snaps width by one grid card and height by one spaced list row', () => {
